...
#include "E:/opencv/Image-Registration/SAR-SIFT-opencv249/Sar_sift.h"
using namespace cv;
using namespace std;
...
/*image表示输入的原始图像
 ...
 orient表示尺度空间像素的梯度方向
 */
void sift::build_sar_sift_space(const Mat& image, vector<Mat>& sar_harris_func, vector<Mat>& amplit, vector<Mat>& orient)
{
	// 转换输入图像格式
	Mat gray_img;
	if (image.channels() != 1)
